...
pub fn make_acceptor() -> SslAcceptor
{
    ...
    let mut file = File::open("tests/certificate.pfx").expect("ssl certificate not found");
    let mut pkcs12 = vec![];
    file.read_to_end(&mut pkcs12).unwrap();
    let pkcs12 = Pkcs12::from_der(&pkcs12).unwrap();
    let identity = pkcs12.parse("asdf").unwrap(); // this is set when you create the password with openssl
